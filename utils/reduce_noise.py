...
print(
    f"Task 0: Original unique counts {np.unique(y0, return_counts=True)}, Denoised unique counts {np.unique(y0_denoised, return_counts=True)}"
)
print(
    f"Task 1: Original unique counts {np.unique(y1, return_counts=True)}, Denoised unique counts {np.unique(y1_denoised, return_counts=True)}"
)
print(f"Task 1: Original size {x1.shape[0]}, Denoised size {x1_denoised.shape[0]}")

np.savez(
    "data/rff/radar/task0.npz",
    xtr=x0_denoised,
    ytr=y0_denoised,
    xte=task0["xte"],
    yte=task0["yte"],
