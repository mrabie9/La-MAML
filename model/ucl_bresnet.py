...
class BayesianClassifier(nn.Module):
    ...
    def __init__(self, n_outputs: int, n_tasks: int, cfg: UCLConfig, args: object | None) -> None:
        super().__init__()
        self.cfg = cfg
        ...
        self.heads = nn.ModuleList(
            [BayesianLinear(self.feature_dim, classes_per_task, ratio=cfg.ratio) for _ in range(n_tasks)]
        )
...
class Net(nn.Module):
    ...
    def __init__(self, n_inputs: int, n_outputs: int, n_tasks: int, args: object) -> None:
        ...
        self.model = BayesianClassifier(n_outputs, n_tasks, self.cfg, args)
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, t: int) -> Tuple[float, float]:
        ...
        if self.split:
            ...
            if (y_local.min() < 0) or (y_local.max() >= self.nc_per_task):
                raise ValueError(
                    f"Labels out of range for task {t}: expected in [0, {self.nc_per_task - 1}] after offset, got "
