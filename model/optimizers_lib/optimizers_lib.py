...
def bgd(model, **kwargs):
    ...
    all_params = [
        {"params": params} for l, (name, params) in enumerate(model.named_parameters())
    ]
...
def sgd(model, **kwargs):
    ...
    all_params = [
        {"params": params, "name": name, "initial_lr": kwargs.get("lr", 0.1)}
...
def adam(model, **kwargs):
    ...
    all_params = [
        {"params": params, "name": name, "initial_lr": kwargs.get("lr", 0.001)}
        for l, (name, params) in enumerate(model.named_parameters())
    ]
...
def adagrad(model, **kwargs):
    ...
    all_params = [
        {"params": params, "name": name, "initial_lr": kwargs.get("lr", 0.01)}
        for l, (name, params) in enumerate(model.named_parameters())
    ]
