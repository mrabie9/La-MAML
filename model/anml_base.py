...
class Learner(nn.Module):
    ...
    def __init__(self, n_outputs, args, neuromodulation=True):
        ...
        for i, (name, param) in enumerate(self.config):
            if "conv" in name:
                cout, cin, k = param[
                    :3
                ]  # ensure your config stores 1D conv as [cout, cin, k, (stride), (pad)]
    ...
    def forward(self, x, vars=None, bn_training=True, feature=False):
        ...
        if self.Neuromodulation:
            ...
            nm_data = F.batch_norm(
                nm_data, running_mean, running_var, weight=w, bias=b, training=True
            )
...
class NeuromodNet1D(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv1d(
            ...
            kernel_size=k,
            stride=stride,
            padding=(k // 2 if pad == "same" else 0),
            bias=True,
        )
    ...
    def features_only(self, x, fw=None):
        ...
        x = F.conv1d(
            x,
            self._W(self.conv1, "weight", fw),
            self._W(self.conv1, "bias", fw),
            stride=self.conv1.stride,
            padding=self.conv1.padding,
            dilation=self.conv1.dilation,
            groups=1,
        )
        x = self._bn(self.bn1, x, fw)
        x = F.relu(x)
        if 1 in self.pool_every:
            x = self.pool(x)

        x = F.conv1d(
            x,
            self._W(self.conv2, "weight", fw),
            self._W(self.conv2, "bias", fw),
            stride=self.conv2.stride,
            padding=self.conv2.padding,
            dilation=self.conv2.dilation,
            groups=1,
        )
        x = self._bn(self.bn2, x, fw)
        x = F.relu(x)
        if 2 in self.pool_every:
            x = self.pool(x)

        x = F.conv1d(
            x,
            self._W(self.conv3, "weight", fw),
            self._W(self.conv3, "bias", fw),
            stride=self.conv3.stride,
            padding=self.conv3.padding,
            dilation=self.conv3.dilation,
            groups=1,
        )
        x = self._bn(self.bn3, x, fw)
        x = F.relu(x)
        if 3 in self.pool_every:
            x = self.pool(x)
